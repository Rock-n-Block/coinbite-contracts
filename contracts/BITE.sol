...
pragma solidity ^0.8.0;
contract BITE is FeeToAddrDeflAutoLPToken, AbstractBurnableDeflToken {
    constructor(
        string memory tName, 
        string memory tSymbol, 
        uint256 totalAmount,
    ...
    mapping (address => TimeAndAmount) public mustHave;
    ...
    function _transfer(address from, address to, uint256 amount) internal override(AbstractDeflationaryAutoLPToken, AbstractDeflationaryToken) {
        require(block.timestamp > mustHave[from].time || (balanceOf(from) - amount) >= mustHave[from].amount, "Cannot transfer this amount due to lock");
        AbstractDeflationaryAutoLPToken._transfer(from, to, amount);
    }
}
